...
class PowerPointAdapter(ABC):
    ...
    @abstractmethod
    def save_presentation(self, presentation_id: str, path: str = None) -> Dict[str, Any]:
        """Save a presentation"""
        pass
    
    @abstractmethod
    def close_presentation(self, presentation_id: str, save: bool = True) -> Dict[str, Any]:
    ...
    @abstractmethod
    def update_text(self, presentation_id: str, slide_id: str, shape_id: str, text: str) -> Dict[str, Any]:
    ...
    @abstractmethod
    def set_slide_title(self, presentation_id: str, slide_id: str, title: str) -> Dict[str, Any]:
...
class WindowsCOMAdapter(PowerPointAdapter):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.ppt_app = self.win32com.client.GetActiveObject("PowerPoint.Application")
            return True
        except:
            try:
...
class CrossPlatformPPTXAdapter(PowerPointAdapter):
    ...
    def set_slide_title(self, presentation_id: str, slide_id: str, title: str) -> Dict[str, Any]:
        ...
        try:
            slide_idx = int(slide_id.strip('"\'')) - 1  # python-pptx uses 0-based indexing
            ...
            if hasattr(slide.shapes, 'title') and slide.shapes.title:
                ...
                title_textbox = slide.shapes.add_textbox(
                    self.Inches(0.5), self.Inches(0.5), 
...
def get_powerpoint_adapter() -> PowerPointAdapter:
    ...
    class UnavailableAdapter(PowerPointAdapter):
        ...
        def get_open_presentations(self): return self._error_response()
        def open_presentation(self, path): return self._error_response()
        def create_presentation(self): return self._error_response()
        def save_presentation(self, presentation_id, path=None): return self._error_response()
        def close_presentation(self, presentation_id, save=True): return self._error_response()
        def get_slides(self, presentation_id): return self._error_response()
        def add_slide(self, presentation_id, layout_type=1): return self._error_response()
        def get_slide_text(self, presentation_id, slide_id): return self._error_response()
        def update_text(self, presentation_id, slide_id, shape_id, text): return self._error_response()
        def add_text_box(self, presentation_id, slide_id, text, left=100, top=100, width=400, height=200): return self._error_response()
        def set_slide_title(self, presentation_id, slide_id, title): return self._error_response()
